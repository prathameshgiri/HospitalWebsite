          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {allServices.map((service, i) => {
              ...
              return (
                <Card
                  key={service.id}
                  className={`group overflow-hidden bg-white hover:shadow-[0_20px_50px_rgba(13,148,136,0.12)] hover:-translate-y-2 rounded-[2rem] transition-all duration-500 relative flex flex-col h-full cursor-pointer ${
                    isSelected 
                      ? "ring-2 ring-medical-500 border-transparent shadow-[0_20px_50px_rgba(13,148,136,0.15)] -translate-y-2" 
                      : "border border-gray-100/80 shadow-md"
                  }`}
                  onClick={() => setSelectedService(isSelected ? null : service.id)}
                >
                  {/* Ambient Glow Backdrop */}
                  <div className={`absolute -right-10 -bottom-10 w-40 h-40 bg-gradient-to-br from-medical-500/10 to-ocean-500/10 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-all duration-700 pointer-events-none group-hover:scale-125 ${
                  ...
                  <CardContent className="p-6 sm:p-8 flex flex-col h-full justify-between relative z-10">
                    <div>
                      ...
                      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-all duration-500 shadow-sm relative overflow-hidden ${
                        isSelected 
                          ? "bg-gradient-to-br from-medical-500 to-ocean-500 text-white scale-110 shadow-lg shadow-medical-500/25" 
                          : "bg-gradient-to-br from-medical-50 to-ocean-50/50 text-medical-600 group-hover:scale-110 group-hover:from-medical-500 group-hover:to-ocean-500 group-hover:text-white group-hover:shadow-lg group-hover:shadow-medical-500/25"
                      }`}>
                        <div className="absolute inset-0 bg-white opacity-0 group-hover:opacity-10 transition-opacity duration-300"></div>
                        <IconComp className="w-7 h-7 transition-colors duration-500 relative z-10" />
                      </div>
                      
                      <h3 className={`text-xl md:text-2xl font-bold font-display transition-colors duration-300 mb-3 ${
                        isSelected ? "text-medical-600" : "text-gray-900 group-hover:text-medical-600"
                      }`}>
                        {service.title}
                      </h3>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-auto">
                        {service.features.map((feat) => (
                          <div key={feat} className={`flex items-center gap-2 text-xs font-semibold px-3 py-2.5 rounded-xl transition-all duration-300 border border-gray-100/50 ${
                            isSelected 
                              ? "bg-medical-50 text-medical-700 border-medical-100/50" 
                              : "text-gray-600 bg-gray-50/80 hover:bg-medical-50/80 hover:text-medical-700 hover:border-medical-100/50"
                          }`}>
                            <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 ${isSelected ? "text-medical-600" : "text-medical-500"}`} />
                      ...
                      <div className="mt-8 pt-5 border-t border-gray-100 flex items-center justify-between">
                        <span className={`text-xs font-bold transition-colors duration-300 ${
                          isSelected ? "text-medical-600" : "text-gray-400 group-hover:text-medical-600"
                        }`}>
                          {isSelected ? "Active Service" : "Explore Service"}
                        </span>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 transform shadow-sm ${
                          isSelected 
                            ? "bg-medical-50 text-medical-600 rotate-90" 
                            : "bg-gray-50 group-hover:bg-medical-50 text-gray-400 group-hover:text-medical-600 group-hover:translate-x-1"
                        }`}>
